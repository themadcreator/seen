 ...
 * projection because Canvas and SVG support only affine transformations. So, in
 * order to fake it, we create an affine transform that approximates the linear
 * effects of a perspective projection on an unrendered planar surface that
 * represents the text's shape. We can use this transform directly in the text
 * painter to warp the text.
 *
...
export const Affine = {
     ...
     *
     * We further re-arranged the rows to avoid having to do any matrix
     * factorization.
     *
     *
     */
     ...
     * triangular) for this matrix, we avoid any matrix factorization and can
     * go directly to back-substitution to solve the matrix equation.
     *
     * To use the affine transform, use the indices like so (note that we flip
     * y):
     *
    ...
    solveForAffineTransform(points: Point[]) {
        ...
        const b = [points[1].x, points[2].x, points[0].x, points[1].y, points[2].y, points[0].y];
