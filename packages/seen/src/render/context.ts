import { ICirclePainter, IPathPainter, IRectPainter, ITextPainter } from "./painters";
...
/**
 * The `RenderContext` uses `RenderModel`s produced by the scene's render method
 * to paint the shapes into an HTML element. Since we support both SVG and
 * Canvas painters, the `RenderContext` and `RenderLayerContext` define a common
 * interface.
 */
export abstract class RenderContext {
    ...
    /**
     * Returns a new `Animator` with this context's render method
     * pre-registered.
     */
    public animate() {
    ...
    /**
     * Add a new `RenderLayerContext` to this context. This allows us to easily
     * stack paintable components such as a fill backdrop, or even multiple
     * scenes in one context.
     */
...
/**
 * The `RenderLayerContext` defines the interface for producing painters that
 * can paint various things into the current layer.
 */
