...
export const Viewports = {
    ...
    center(width = 500, height = 500, x = 0, y = 0): IViewport {
        ...
        const postscale = new Matrix().scale(width, -height, height).translate(x + width / 2, y + height / 2, height);
    ...
    origin(width = 500, height = 500, x = 0, y = 0): IViewport {
        ...
        const postscale = new Matrix().scale(width, -height, height).translate(x, y, 0);
        return { prescale, postscale };
    },
};
...
export class Camera extends Transformable implements ICameraOptions {
    ...
    constructor(options: Partial<ICameraOptions> = {}) {
        super();
        Util.defaults<ICameraOptions>(this, options, Camera.defaults());
    }
}
