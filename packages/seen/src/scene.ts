...
import { Camera, Viewports, IViewport } from "./camera";
...
export class Scene implements ISceneOptions {
  ...
  public render() {
    ...
    this.model.eachRenderable(
      ...
      (shape: Shape, lights: Light[], transform: Matrix) => {
        for (const surface of shape.surfaces) {
          ...
          const renderModel = this._renderSurface(
            surface,
            transform,
            projection,
            viewport
    ...
    // surfaces farthest from the eye are painted first. (Painter's Algorithm)
  ...
  // remove many shapes from the scene's models since this cache has no eviction
  // policy.
  public flushCache() {
