...
seen.Mocap = class Mocap {
  ...
  createMocapModel(shapeFactory) {
    if (shapeFactory == null) { shapeFactory = seen.Mocap.DEFAULT_SHAPE_FACTORY; }
  ...
  _applyChannelTransform(channel, m, v) {
    switch (channel) {
      case 'Xposition': m.translate(v, 0, 0); break;
      case 'Yposition': m.translate(0, v, 0); break;
      case 'Zposition': m.translate(0, 0, v); break;
      case 'Xrotation': m.rotx((v * Math.PI) / 180.0); break;
      case 'Yrotation': m.roty((v * Math.PI) / 180.0); break;
      case 'Zrotation': m.rotz((v * Math.PI) / 180.0); break;
    }
    return m;
  }
  ...
  _attachJoint(model, joint, joints, shapeFactory) {
    ...
    if (joint.joints != null) {
      ...
      for (let child of Array.from(joint.joints)) {
        ...
        const p = seen.P(child.offset != null ? child.offset.x : undefined, child.offset != null ? child.offset.y : undefined, child.offset != null ? child.offset.z : undefined);
        childShapes.add(shapeFactory(joint, p));

        // Recurse with a new model for any child joints
        if (child.type === 'JOINT') { this._attachJoint(childShapes.append(), child, joints, shapeFactory); }
      }
    }
  }
};
