...
export const Util = {
  ...
  defaults<T>(obj: T, opts: Partial<T>): void {
  ...
  // // Copies default values. First, overwrite undefined attributes of `obj` from
  // // `opts`. Second, overwrite undefined attributes of `obj` from `defaults`.
  // defaults(obj: any, opts: any, defaults: any) {
  //   for (var prop in opts) {
  //     if ((obj[prop] == null)) { obj[prop] = opts[prop]; }
  //   }
  //   return (() => {
  //     const result = [];
  //     for (prop in defaults) {
  //       if ((obj[prop] == null)) { result.push(obj[prop] = defaults[prop]); } else {
